...
final class CoachSnapshotBuilderTests: XCTestCase {
    ...
    func test_charter_namesIdentityStandardsAndSafety() {
        ...
        XCTAssertLessThan(charter.count, 10_000)
        // The eval run's tics, named in the charter so the model can avoid them.
        XCTAssertTrue(charter.contains("VOICE"))
        XCTAssertTrue(charter.contains("never by paraphrasing"))
        XCTAssertTrue(charter.contains("never ask \"what part of\" twice"))
        XCTAssertTrue(charter.contains("WRITING FOR THEM"))
        XCTAssertTrue(charter.contains("Most replies use no callback"))
        XCTAssertTrue(charter.contains("120 to 220 words"))
        ...
        XCTAssertTrue(CoachCharter.acquaintanceContract(emptyFiles: ["People", "Body & health"]).contains("FILES STILL EMPTY (ask toward the first one next): People, Body & health."))
