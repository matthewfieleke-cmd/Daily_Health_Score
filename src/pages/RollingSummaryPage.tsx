...
type RollingSummaryPageProps = {
  days: number;
};

export function RollingSummaryPage({ days }: RollingSummaryPageProps) {
  const version = useRecordsVersion();
  const records = useMemo(() => {
    void version;
    return loadRecords();
  }, [version]);
  const keys = getDateKeysForRollingWindow(days);
  const stats = getRollingAverages(records, keys);

  if (!stats) {
    return (
      <div className="page-content">
        <p className="muted">No records in the last {days} days yet.</p>
      </div>
    );
  }
  ...
  return (
    <div className="page-content">
      <header className="section-head">
        <h1 className="page-title">{days}-Day view</h1>
        <p className="muted">
