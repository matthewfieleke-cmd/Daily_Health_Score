...
export function advanceSuggestion(
  ...
  state: UsedSuggestionsState,
): { text: string; nextState: UsedSuggestionsState } {
  const cat: SuggestionCategory =
    primaryFocus === "maintain" ? "maintain" : primaryFocus;
  const pool = LIBRARIES[cat];
  let nextState = state;
  let unused = pool.filter((e) => !nextState[cat].includes(e.id));
  if (unused.length === 0) {
    nextState = { ...nextState, [cat]: [] };
    unused = pool;
  }
  const choice = unused[0]!;
  nextState = { ...nextState, [cat]: [...nextState[cat], choice.id] };
  return { text: choice.text, nextState };
}
